<!-- Add Portfolio Modal -->
<div id="editPortfolioModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 transition-opacity bg-white/0 bg-opacity-75 backdrop-blur-md" 
             onclick="closeModal('editPortfolioModal')"></div>
        ...
        <div class="inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white/10 backdrop-blur-lg shadow-xl rounded-2xl border border-white/20">
            ...
            <!-- Modal Form -->
            <form method="POST" action="{{ route('photographer.portfolio.update', $item->id) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const form = document.querySelector('#editPortfolioModal form');
    if (form) {
    ...
    const dropZone = document.querySelector('#editPortfolioModal .border-dashed');
    if (dropZone) {
...
#editPortfolioModal .form-input:focus {
    outline: none;
    border-color: rgba(59, 130, 246, 0.5);
    box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1);
}

#editPortfolioModal .form-input::placeholder {
    color: rgba(255, 255, 255, 0.5);
}

#editPortfolioModal select.form-input option {
    background: #1f2937;
    color: white;
}

/* Drag and drop animation */
#editPortfolioModal .border-dashed {
    transition: all 0.3s ease;
}

/* Checkbox styling */
#editPortfolioModal input[type="checkbox"] {
    background: rgba(255, 255, 255, 0.1);
    border: 1px solid rgba(255, 255, 255, 0.3);
}

#editPortfolioModal input[type="checkbox"]:checked {
    background: rgb(59, 130, 246);
    border-color: rgb(59, 130, 246);
}

/* Date input styling */
#editPortfolioModal input[type="date"] {
    color-scheme: dark;
}

/* Responsive adjustments */
@media (max-width: 640px) {
    #editPortfolioModal .inline-block {
        margin: 1rem;
        width: calc(100% - 2rem);
    }
}
